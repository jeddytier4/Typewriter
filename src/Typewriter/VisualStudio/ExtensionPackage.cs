...
namespace Typewriter.VisualStudio
{
    ...
    [InstalledProductRegistration("#110", "#112", "10.22.0", IconResourceID = 401)]
    ...
    public sealed class ExtensionPackage : AsyncPackage, IDisposable
    {
        ...
        protected override async Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
        {
            ...
            await GetDte().ConfigureAwait(false);
            await GetStatusBar().ConfigureAwait(false);
        ...
        private void WireUpEvents()
        {
            ...
            _documentEvents.DocumentSaved += document =>
            {
                ...
                else if (document.FullName.EndsWith(Constants.TstTemplateExtension, StringComparison.InvariantCultureIgnoreCase)||document.FullName.EndsWith(Constants.TstXTemplateExtension, StringComparison.InvariantCultureIgnoreCase))
                {
        ...
        private static void RegisterIcons()
        {
            try
            {
                ...
                using (RegistryKey classes = Registry.CurrentUser.OpenSubKey("SoftWare\\Classes", true))
                {
                    ...
                    using (var key = classes.CreateSubKey(Constants.TstTemplateExtension + "\\DefaultIcon"))
                    {
                        key?.SetValue(string.Empty, path);
                    }
                    using (var key = classes.CreateSubKey(Constants.TstXTemplateExtension + "\\DefaultIcon"))
                    {
                        key?.SetValue(string.Empty, path);
                    }
                }
